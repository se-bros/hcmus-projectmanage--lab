...
2.  Backend LibDMS tiếp nhận file, lưu tệp gốc vào trường `file_data` (cột BYTEA), tạo bản ghi tài liệu ở trạng thái "Processing" trong **PostgreSQL**.
3.  Backend gửi một message chứa thông tin file vào hàng đợi (Message Queue).
4.  **OCR & Converter Worker** nhận message, truy vấn và tải ảnh từ trường BYTEA của PostgreSQL xuống, gọi **Tesseract OCR** để trích xuất text tiếng Việt.
5.  Worker sắp xếp cấu trúc text, gọi **Calibre/Pandoc** đóng gói thành file EPUB. File EPUB mới sinh được cập nhật ngược lại vào PostgreSQL dưới dạng cột BYTEA.
