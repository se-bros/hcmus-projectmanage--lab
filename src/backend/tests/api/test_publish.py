...
def test_publish_reports_all_missing_metadata_without_creating_job(api_context) -> None:
...
def test_publish_rejects_missing_or_blank_pages(api_context) -> None:
    client, testing_session, _ = api_context
    with testing_session() as db:
        document = _document()
        db.add(document)
        db.commit()
        document_id = document.id
    no_pages = client.post(f"/documents/{document_id}/publish")
    ...
    with testing_session() as db:
        ...
        db.commit()
    blank_page = client.post(f"/documents/{document_id}/publish")
    assert blank_page.status_code == 422
    assert blank_page.json()["detail"]["missing_page_numbers"] == [1]


def test_publish_creates_pending_job_and_rejects_active_duplicate(api_context) -> None:
    client, testing_session, _ = api_context
    with testing_session() as db:
        document = _document()
        db.add(document)
    ...
    assert client.post(f"/documents/{document_id}/publish").status_code == 409
