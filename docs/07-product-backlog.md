...
**Definition of Ready (DoR):** Card chỉ được kéo vào *In Progress* khi có ID, AC rõ ràng, `depends_on` đã Done (nếu có), size S hoặc M (≤ 2 ngày). Card quá to phải tách trước.
...
Mục tiêu: *API chạy được, upload file, OCR trả text theo trang.*
...
Mục tiêu: *Sửa text trên UI, gán metadata, xuất EPUB, mở reader.*
...
Mục tiêu: *Đăng nhập, phân quyền, tìm kiếm, danh sách tài liệu.*
...
Mục tiêu: *Split-screen, signed URL, Google OAuth, tùy chỉnh reader.*
...
Mục tiêu: *Tags, highlight, citation, search nâng cao.*
