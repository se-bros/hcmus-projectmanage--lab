...
```mermaid
flowchart LR
    %% Tự vẽ sơ đồ so sánh quy trình As-Is vs To-Be
    subgraph AsIs["Quy trình cũ (As-Is)"]
        A1["Scan"] --> A2["Lưu ảnh thô"] --> A3["Upload GDrive"]
    end
    subgraph ToBe["Quy trình mới (To-Be)"]
        B1["Upload"] --> B2["OCR tự động"] --> B3["Biên tập"] --> B4["EPUB DRM"]
    end
```
...
```mermaid
flowchart TD
    %% Tự vẽ sơ đồ phân rã Backlog
    Backlog["Product Backlog (26 Stories)"] --> Epic["4 Epics"]
    Epic --> Story["User Story + Acceptance Criteria"]
    Story --> DoD["Definition of Done (DoD 5 tiêu chí)"]
```
...
```mermaid
flowchart TD
    %% Tự vẽ sơ đồ Change Request
    CR["Gửi yêu cầu Change Request"] --> Impact["Đánh giá Tác động"]
    Impact --> Approve{"Phê duyệt?"}
    Approve -->|Yes| Update["Bổ sung Ngân sách/Hạn chót & Phụ lục SoW"]
    Approve -->|No| Reject["Giữ nguyên SoW ban đầu"]
```
